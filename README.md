...
1) Multiple full passes over the text (cache inefficiency)  
...
2) Excessive allocations and cloning (function call + heap overhead)  
...
3) O(n²) top-10 algorithm (poor algorithm choice)  
...
4) Rebuilding all words to find the longest (wasteful memory traffic)  
